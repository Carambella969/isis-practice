...
async function editItem(type, id) {
    console.log('Редактирование:', type, id); // Для отладки
    ...
    try {
        ...
        console.log('Полученные данные:', item); // Для отладки
        ...
        alert('Не удалось загрузить данные для редактирования. Ошибка: ' + error.message);
    }
}

// ========== УДАЛЕНИЕ ==========
async function deleteItem(type, id) {
    console.log('Удаление:', type, id); // Для отладки
    
    if (!confirm(`Вы уверены, что хотите удалить эту запись из таблицы "${type}"?`)) return;
...
document.getElementById('dynamicForm').addEventListener('submit', async (e) => {
    ...
    console.log('Сохранение:', editingType, data); // Для отладки
            ...
            return `
                ...
                <div class="form-group"><label>Тип:</label>
                    <select name="type">
                        ...
                        <option value="шерстяная" ${data.type === 'шерстяная' ? 'selected' : ''}>Шерстяная</option>
                    </select>
                </div>
            ...
            return `
                ...
                <div class="form-group"><label>Метод оплаты:</label>
                    <select name="payment_method">
                        ...
                        <option value="карта" ${data.payment_method === 'карта' ? 'selected' : ''}>Карта</option>
                    </select>
                </div>
            `;
        default:
